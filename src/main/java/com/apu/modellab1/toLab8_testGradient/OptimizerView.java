...
public class OptimizerView extends javax.swing.JPanel {
	...
	private javax.swing.DefaultComboBoxModel getDefaultComboBoxModel() {
		if (ivjDefaultComboBoxModel == null) {
			try {
				...
				// user code begin {1}
				ivjDefaultComboBoxModel.addElement("Полный");
				ivjDefaultComboBoxModel.addElement("Дробный + 1");
				ivjDefaultComboBoxModel.addElement("Дробный - 1");
				// user code end
	...
	private Optimizer getOptimizer() {
		if (ivjOptimizer == null) {
			try {
				ivjOptimizer = new com.apu.modellab1.toLab8_testGradient.Optimizer();
				// user code begin {1}
				// user code end
			} catch (java.lang.Throwable ivjExc) {
				// user code begin {2}
